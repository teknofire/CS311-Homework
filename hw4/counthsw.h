...
public:
	...
	int total_nodes();
	int countHSW_recurse(const Coord & node, int nodes_left);

private:
	int & get(const Coord & node);
	bool been_visited(const Coord & node);
	void visit(const Coord & node);
	void unvisit(const Coord & node);
	bool column_orphaned_near(const Coord & node);
	bool row_orphaned_near(const Coord & node);
	bool valid_node(const Coord & node);

	Coord _size;
	Coord _finish;

	static const std::vector<Coord> adjacent_dirs;
